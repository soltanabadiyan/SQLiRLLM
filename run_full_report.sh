...
SEED="${SEED:-42}"
SKIP_SIM="${SKIP_SIM:-0}"
LIVE_TARGETS="${LIVE_TARGETS:-dvwa_sqli dvwa_sqli_medium dvwa_sqli_hard dvwa_sqli_max dvwa_waf sqli_labs_1 sqli_labs_11 bwapp_sqli juiceshop_login}"
...
def validated_rate(rows, detected_key):
  clean = [r for r in rows if not r.get("error")]
  if not clean:
    return 0.0, 0.0
  expected = [bool(r.get("expected_vulnerable", True)) for r in clean]
  detected = [bool(r.get(detected_key, False)) for r in clean]
  validated = sum(1 for exp, det in zip(expected, detected) if exp == det) / len(clean)
  positives = [det for exp, det in zip(expected, detected) if exp]
  coverage = (sum(positives) / len(positives)) if positives else 0.0
  return validated, coverage

sqlmap_validated, sqlmap_coverage = validated_rate(sqlmap_rows, "vulnerable_detected")
sqli_validated, sqli_coverage = validated_rate(
  [{**r, "detected": r.get("strategies_succeeded", 0) > 0} for r in sqli_rows],
  "detected",
)
...
- SQLMap validated success rate: {sqlmap_validated:.3f}
- SQLMap vulnerable-target coverage: {sqlmap_coverage:.3f}
- SQLiRLLM validated success rate: {sqli_validated:.3f}
- SQLiRLLM vulnerable-target coverage: {sqli_coverage:.3f}
...
report_json.write_text(json.dumps({
  ...
  "run_config": run_cfg,
    ...
    "live": {
      "SQLMap": {"detected": sqlmap_det, "total": sqlmap_total, "rate": round(sqlmap_rate, 4), "validated_success_rate": round(sqlmap_validated, 4), "coverage_on_vulnerable": round(sqlmap_coverage, 4)},
      "SQLiRLLM": {"detected": sqli_det, "total": sqli_total, "rate": round(sqli_rate, 4), "validated_success_rate": round(sqli_validated, 4), "coverage_on_vulnerable": round(sqli_coverage, 4)},
    },
    "artifacts": {
